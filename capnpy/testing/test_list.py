...
def test_list_of_structs():
    ...
    def read_point(i):
        ...
        x = p._read_data(0, Types.int64)
        y = p._read_data(8, Types.int64)
        return x, y
...
def test_list_comparisons():
    ...
    buf2 = 'garbage0' + buf1
    #
    lst1 = PrimitiveList.from_buffer(buf1, 0, None, ListPtr.SIZE_64, 4, Types.int64)
    lst2 = PrimitiveList.from_buffer(buf2, 8, None, ListPtr.SIZE_64, 4, Types.int64)
    lst3 = PrimitiveList.from_buffer(buf1, 0, None, ListPtr.SIZE_64, 3, Types.int64)
    #
    assert lst1 == lst2
...
def test_far_pointer():
    ...
    seg1 = ('\x00\x00\x00\x00\x00\x00\x00\x00'    # random data
            ...
            '\x04\x00\x00\x00\x00\x00\x00\x00')   # 4
    buf = seg0+seg1
    blob = Blob(buf, 8, segment_offsets=(0, 16))
    lst = blob._read_list(0, PrimitiveList, Types.int64)
    assert lst == [1, 2, 3, 4]
