...
class TestConstructors(CompilerTest):
    ...
    def test_text(self):
        ...
        assert foo._buf.buf == ('\x01\x00\x00\x00\x00\x00\x00\x00'
    ...
    def test_struct(self):
        ...
        p = mod.Point(1, 2)
        foo = mod.Foo(p)
        assert foo._buf.buf == ('\x00\x00\x00\x00\x02\x00\x00\x00'  # ptr to point
    ...
    def test_list(self):
        ...
        mod = self.compile(schema)
        foo = mod.Foo([1, 2, 3, 4])
        assert foo._buf.buf == ('\x01\x00\x00\x00\x22\x00\x00\x00'   # ptrlist
    ...
    def test_list_of_void(self):
        ...
        mod = self.compile(schema)
        foo = mod.Foo([None]*4)
        assert foo._buf.buf == ('\x01\x00\x00\x00\x20\x00\x00\x00')  # ptrlist
