...
def test_struct():
    ...
    r = Rectangle.from_buffer(buf, 0, 0, 2)
    assert r.a.x == 1
    assert r.a.y == 2
    assert r.b.x == 3
    assert r.b.y == 4
...
def test_void():
    ...
    f = Foo.from_buffer('somedata', 0, 0, 0)
...
def test_read_group():
    ...
    #
    r = Rectangle.from_buffer(buf, 8, 4, 0)
    assert r.a.x == 1
    assert r.a.y == 2
    assert r.b.x == 3
    assert r.b.y == 4
...
def test_group_union():
    ...
    shape = Shape.from_buffer(buf, 0, 3, 0)
...
def test_anyPointer():
    ...
    f = Foo.from_buffer('somedata', 0, 0, 1)
